...
class Texture3D(
    ...
    init {
        ...
        image.onload = {
            ...
            gl.texImage3D( target = GL.TEXTURE_3D, level = 0, internalformat = GL.R8,
             width = this.width, height = this.height, depth = this.depth, border = 0, format = GL.RED, type = GL.UNSIGNED_BYTE, source = data3d )
            ...
            gl.texParameteri( target = GL.TEXTURE_3D, pname = GL.TEXTURE_WRAP_R, param = GL.CLAMP_TO_EDGE)
            gl.texParameteri( target = GL.TEXTURE_3D, pname = GL.TEXTURE_WRAP_S, param = GL.CLAMP_TO_EDGE)
            gl.texParameteri( target = GL.TEXTURE_3D, pname = GL.TEXTURE_WRAP_T, param = GL.CLAMP_TO_EDGE)
