...
class Vec4Array(backingStorage: Float32Array?, startIndex: Int = 0, endIndex: Int = 0) : VecArray() {
  ...
  override fun set(vararg values : Float) : Vec4Array {
      ...
      storage[current++] = values.getOrNull(0) ?: 0.0f
      storage[current++] = values.getOrNull(1) ?: 0.0f
      storage[current++] = values.getOrNull(2) ?: 0.0f
      storage[current++] = values.getOrNull(3) ?: 1.0f
